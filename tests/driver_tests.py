...
class TestDriver(unittest.TestCase):
    ...
    def test_request_with_additional_curl_options(self, mock_pycurl, mock_BytesIO):
        ...
        _ = driver.request("endpoint", {"p": "v"}, (("a", 1), ("b", 2), ("c", 4)))
    ...
    def test_request(self, mock_pycurl, mock_BytesIO):
        ...
        assert_equal(result, bio)
        assert_equal(result.getvalue(), expected_html)
        assert_equal(1, driver.request_count)

        driver.request("endpoint", {"v": "p"})

        assert_equal(2, driver.request_count)
    ...
    def test_initialize_connects_to_initialize(self, mock_initialize):
        ...
        assert_equal(driver.use_lxml, asXML)
    ...
    def test_call_populi(self, mock_request):
        ...
        assert_equal(result_byte, bio)

    @patch("populi.driver.request")
    def test_call_populi_raises_authentication_error(self, mock_request):
        bio = BytesIO()
        bio.write(
            b'<?xml version="1.0" encoding="ISO-8859-1"?><error><code>AUTHENTICATION_ERROR</code><message>Malformed access key</message></error>'
        )
        bio.seek(0)
        mock_request.return_value = bio
        driver.driver.access_key = "override"
        ...
        try:
            ...
            assert_equal("Malformed access key", str(e))
    ...
    def test_call_populi_for_raw_data(self, mock_request):
        ...
        assert_equal(b"HELLO WORLD", raw_data.read())
        assert_equal(None, other)

    @patch("populi.driver.request")
    def test_call_populi_raises_unknown_task(self, mock_request):
        bio = BytesIO()
        bio.write(
            b'<?xml version="1.0" encoding="ISO-8859-1"?><error><code>UNKNOWN_TASK</code><message>Unknown Task</message></error>'
        )
        bio.seek(0)
        mock_request.return_value = bio
        driver.driver.access_key = "override"
        ...
        try:
            driver.driver.call_populi({"a": "b"})
            assert False, "Should have raised UnknownTask"
        except exceptions.UnknownTask as e:
            assert_equal("Unknown Task", str(e))

    @patch("populi.driver.request")
    def test_call_populi_raises_other_when_unknown(self, mock_request):
        bio = BytesIO()
        bio.write(
            b'<?xml version="1.0" encoding="ISO-8859-1"?><error><code>WHATEVER_TASK</code><message>Unknown Task</message></error>'
        )
        bio.seek(0)
        mock_request.return_value = bio
        driver.driver.access_key = "override"
        ...
        try:
            driver.driver.call_populi({"a": "b"})
            assert False, "Should have raised Other Error"
    ...
    def test_get_all_anonymous_returns_correct_encoding(self, mock_call_populi):
        xml_file_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "data/test.xml"
        )
        ...
        assert_equal(
            etree.tostring(xml, xml_declaration=True, encoding="UTF-8").decode("UTF-8"),
            result,
        )
